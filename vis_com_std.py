...
def analyze_hidden_states(hidden_states_p, hidden_states_r):
    std_p = [float(sum(p.std(dim=0))) for p in hidden_states_p]
    std_r = [float(sum(p.std(dim=0))) for p in hidden_states_r]
        ...
        for j, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                for k in range(len(labels)):
                    ...
                    eq_position = input_ids_i[0].cpu().numpy().tolist().index(eq_token)
                    ...
                    hidden_states_p = [p[:, eq_position, :] for p in output_p["hidden_states"]]
                    ...
                    hidden_states_r = [p[:, eq_position, :] for p in output_r["hidden_states"]]
                    std_diff = analyze_hidden_states(hidden_states_p, hidden_states_r)
...
def visualize_wrap(ckpt_path, dataset_path, rtype=1, limit=20):
    ...
    model.to(device)
    dataloader_train_com = DataLoader(
        EvalDataset(dataset_path, GPT2Tokenizer.from_pretrained('gpt2'), ftype='train_com'), batch_size=32,
    ...
    # print(f"{args.dataset_path},Train accuracy: {evaluate(model, dataloader_train_com)}")
    # print(f"{args.dataset_path}, Test_com accuracy: {evaluate(model, dataloader_eval_com)}")
    diff_train = visualize(model, dataloader_train_com, n=n, rtype=rtype, limit=limit)
    diff_test = visualize(model, dataloader_eval_com, n=n, rtype=rtype, limit=limit)
    return {"diff_train": diff_train, "diff_test": diff_test}
...
def run(data_prefix):
    for p in [7]:
        for rtype in [1, 2, 3]:
            ...
            vis_array(diff_test, f"results/{p}_{rtype}_com_diff_test.png")
